/* --- CONFIGURATION & STATE --- */
let examQuestions = { math: [], english: [], economics: [] };
let currentSubject = 'math';
let currentIdx = { math: 0, english: 0, economics: 0 }; 
let userAnswers = { math: {}, english: {}, economics: {} };
...
window.onblur = function() {
    securityWarnings++;
    if(securityWarnings >= 5) {
        alert("SECURITY BREACH: Multiple tab switches detected. Auto-submitting...");
        finishExam();
    } else {
        alert(`SECURITY WARNING: You left the exam screen! (${securityWarnings}/5 Warnings)`);
    }
};
...
function startExam() {
    ...
    const counts = { math: 40, english: 30, economics: 30 };
    ...
    // Safety check for controls element
...
function renderQuestion() {
    ...
    // Logic: Only show Submit on the very last question of Economics
    const isLastSubject = (sub === 'economics'); 
    
    document.getElementById('next-btn').classList.toggle('hidden', isLastOfCurrent);
    document.getElementById('submit-btn').classList.toggle('hidden', !(isLastSubject && isLastOfCurrent));
}
...
function finishExam() {
    clearInterval(timerInterval);
    // Fixed: Changed 'chemistry' to 'economics'
    let scores = { math: 0, english: 0, economics: 0 };
    
    // Calculate Scores
    for(let sub in scores) {
        if(examQuestions[sub]) {
    ...
    const total = scores.math + scores.english + scores.economics;
    const max = examQuestions.math.length + examQuestions.english.length + examQuestions.economics.length;
    const name = document.getElementById('student-name').value;
    ...
    document.getElementById('res-economics').innerText = `${scores.economics}/${examQuestions.economics.length}`;
    ...
    finalReport = `*OFFICIAL MOCK RESULT*%0A` +
                  ...
                  `*Economics:* ${scores.economics}/${examQuestions.economics.length}%0A%0A` +
